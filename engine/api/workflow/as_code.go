...
func UpdateWorkflowAsCode(ctx context.Context, store cache.Store, db gorp.SqlExecutor, proj *sdk.Project, wf sdk.Workflow, branch string, message string, u *sdk.User) (*sdk.Operation, error) {
	if err := RenameNode(db, &wf); err != nil {
		return nil, err
	}
	if err := IsValid(ctx, store, db, &wf, proj, u, LoadOptions{DeepPipeline: true}); err != nil {
		return nil, err
	}
...
func MigrateWorkflowAsCode(ctx context.Context, db *gorp.DbMap, store cache.Store, proj *sdk.Project, wf *sdk.Workflow, encryptFunc sdk.EncryptFunc, u *sdk.User) (*sdk.Operation, error) {
	// Get repository
