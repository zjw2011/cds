...
func (api *API) postWorkflowAsCodeHandler() service.Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		u := deprecatedGetUser(ctx)
		p, err := project.Load(api.mustDB(), api.Cache, key, u,
			project.LoadOptions.WithApplicationWithDeploymentStrategies,
			project.LoadOptions.WithPipelines,
			project.LoadOptions.WithEnvironments,
			project.LoadOptions.WithIntegrations,
		...
		wfDB, err := workflow.Load(ctx, api.mustDB(), api.Cache, p, workflowName, u, workflow.LoadOptions{})
		if err != nil {
			return err
		}
		...
		ope, err := workflow.UpdateWorkflowAsCode(ctx, api.Cache, api.mustDB(), p, wk, branch, message, u)
		if err != nil {
			return err
		}
...
func (api *API) postMigrateWorkflowAsCodeHandler() service.Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
		...
		ope, err := workflow.MigrateWorkflowAsCode(ctx, api.mustDB(), api.Cache, proj, wf, project.EncryptWithBuiltinKey, u)
		if err != nil {
